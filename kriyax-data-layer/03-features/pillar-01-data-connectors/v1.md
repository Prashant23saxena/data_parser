...
| F-DC-007 | Incremental/delta sync | should | F-DC-006 |
| F-DC-008 | Manage saved connections | should | F-DC-004 |
...
**Definition of done:** System produces a column list with inferred types from any uploaded CSV/Excel file. User can see and override detected types before import.
...
**Description:** Before committing data to the database, show the user the first 50-100 rows along with the detected schema (column names + types). User confirms or adjusts before proceeding.

**Priority:** must

**Dependencies:** F-DC-002

**Definition of done:** User sees a table preview with correct columns and types, can rename columns or change types, and clicks "Import" to commit. Can cancel without side effects.
